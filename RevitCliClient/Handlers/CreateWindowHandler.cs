...
{
    public class CreateWindowHandler : ICliCommand
    {
        public string CommandName => "create_window";
        public string Description => "Create window on wall";
        public string Usage => "create_window -w <id> --family-type-id <id> --location-x <x> --location-y <y> [--sill-height <mm>]";
        public CommandCategory Category => CommandCategory.Create;
        public string[] Examples => new[] { "RevitCliClient.exe create_window -w 599906 --family-type-id 950368 --location-x 2500 --location-y 0" };
